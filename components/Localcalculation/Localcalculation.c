...
//窗帘高度H
#define TOTAL_HEIGHT   1850
//进光量x
#define LIGHT_IN   500
//内窗洞口深度z1
#define WINDOW_WIDTH1   0
//外窗洞口深度z2
#define WINDOW_WIDTH2   172
//窗洞口深度修正
#define WINDOW_WIDTH_Mod   0
//窗下墙高度Y
                      ...
                      int* Height,int* Angle)
{
    ...
    //    float G2;//G2高度无直射修正
        ...
        G3=(LIGHT_IN+WINDOW_WIDTH1)*tan(radians(Ab));
        ESP_LOGI(TAG, "G3=%f",G3);
        /*********遮阳下降高度计算G4***************************************************************/
        if((G3-WINDOW_WALL)>TOTAL_HEIGHT)
        {
            G4=0;
        }
        else
        ...
        ESP_LOGI(TAG, "G4=%f",G4);
        /*********入射角修正G5(含窗洞口修正和窗台遮阳控制)***************************************************************/
        // ESP_LOGI(TAG, "Ab=%f",Ab);
        // test=degrees(atan((float)(TOTAL_HEIGHT+WINDOW_WALL)/(float)(LIGHT_IN+WINDOW_WIDTH1+WINDOW_WIDTH2)));
        // ESP_LOGI(TAG, "test=%f",test);
        if(Ab>degrees(atan((float)(TOTAL_HEIGHT+WINDOW_WALL)/(float)(LIGHT_IN+WINDOW_WIDTH1+WINDOW_WIDTH2))))
        {
                G5=0;
        }
        else
        {
            if(Ab<degrees(atan((float)WINDOW_WALL/(float)LIGHT_IN)))
            {
                G5=(float)TOTAL_HEIGHT-(float)(WINDOW_WIDTH1-WINDOW_WIDTH_Mod)*tan(radians(Ab));
            }
            else
            {
                G5=G4;
